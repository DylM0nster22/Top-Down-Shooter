...
die() {
  this.active = false;
  const baseValue = 
      this instanceof FastEnemy ? 5 :
      this instanceof TankEnemy ? 15 :
      this instanceof ShooterEnemy ? 10 :
      this instanceof EliteFastEnemy ? 25 :
      this instanceof EliteTankEnemy ? 35 :
      this instanceof EliteShooterEnemy ? 30 :
      this instanceof BomberEnemy ? 20 :
      this instanceof TeleporterEnemy ? 15 :
      this instanceof PulsarEnemy ? 25 : 5;
  
  const currencyAmount = Math.floor(baseValue * this.player.stats.currencyMultiplier);
...
class TankEnemy extends Enemy {
  constructor(x, y, player) {
    ...
    draw(ctx) {
        ...
        ctx.moveTo(this.x + radius + radius * Math.cos(0), this.y + radius + radius * Math.sin(0));
        for (let i = 1; i <= sides; i++) {
            const angle = i * 2 * Math.PI / sides;
            ctx.lineTo(this.x + radius + radius * Math.cos(angle), 
...
class BomberEnemy extends Enemy {
  constructor(x, y, player, game) {  // Add game parameter
    ...
    this.game.enemies.forEach(enemy => {
        if (enemy !== this) {
            ...
            if (distance < this.explosionRadius) {
                const damage = Math.floor((1 - distance/this.explosionRadius) * this.explosionDamage);
...
class PulsarEnemy extends Enemy {
  ...
  draw(ctx) {
      // Pulse wave effect
      ctx.beginPath();
      ctx.arc(this.x + this.width/2, this.y + this.height/2, this.pulseRadius, 0, Math.PI * 2);
      ctx.strokeStyle = `rgba(75, 0, 130, ${0.5 - (this.pulseRadius/this.maxPulseRadius)/2})`;
      ctx.stroke();
