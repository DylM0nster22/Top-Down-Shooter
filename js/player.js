...
drawXPBar(ctx) {
  ...
  ctx.fillRect(this.x - this.width/2, this.y - 15, barWidth * xpPercent, barHeight);
}
...
update(keys, canvas) {
  ...
  if (this.stats.healthRegen > 0) {
    this.health = Math.min(this.maxHealth, Math.round((this.health + this.stats.healthRegen / 60) * 10) / 10);
}
  ...
  this.dashParticles = this.dashParticles.filter(particle => particle.life > 0);
